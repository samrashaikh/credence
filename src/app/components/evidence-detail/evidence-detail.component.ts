      ...
      <div class="space-y-6 pb-20">
        ...
        >
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-semibold text-slate-300 truncate max-w-xs">{{
                art.title
              }}</span>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              @if (art.provenanceType === 'source_backed') {
                <span
                  class="px-2.5 py-0.5 text-xs font-bold rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1.5 font-mono"
                >
                ...
                <span
                  class="px-2.5 py-0.5 text-xs font-bold rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1.5 font-mono"
                >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div class="lg:col-span-5 space-y-3">
            ...
            <div
              class="bg-slate-800/50 rounded-2xl border border-slate-700/50 overflow-hidden flex flex-col h-[680px] shadow-md"
            >
              ...
              @if (art.fileMetadata.mimeType.startsWith('image/') && art.fileMetadata.storageUrl) {
                ...
                <!-- Text Artifact -->
                <div
                  class="p-4 overflow-y-auto flex-1 font-mono text-xs text-slate-300 leading-relaxed space-y-1 bg-slate-950/80 select-text"
                >
                  @for (line of getSourceLines(art.rawContentText); track $index) {
                    <div
                      class="flex items-start gap-3 hover:bg-slate-900/50 py-0.5 px-1 rounded transition-colors group"
                    >
                      <span
                        class="text-[10px] text-slate-600 font-mono select-none w-6 text-right shrink-0"
                      >
                        {{ $index + 1 }}
                      </span>
              ...
              <div
                class="p-3 bg-slate-900/80 border-t border-slate-700/50 flex items-center justify-between text-[10px] text-slate-400 font-mono"
              >
                <span
                  >Preserved at {{ art.fileMetadata.preservationTimestamp | date: 'medium' }}</span
                >
                <!-- <span class="text-emerald-400 font-bold">INTEGRITY VERIFIED</span> -->
              </div>
            </div>
          </div>
            ...
            @if (art.profile; as prof) {
              ...
              >
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      class="text-xs font-bold uppercase tracking-wider text-purple-300 font-mono"
                      >Gemini 2.5 Flash Analysis</span
                    >
                  </div>
                  <span class="text-[10px] text-slate-400 font-mono">{{
                    prof.extractedAt | date: 'mediumTime'
                  }}</span>
                </div>
                ...
                <div class="p-5 space-y-4 bg-slate-900/30">
                  <div class="flex items-start justify-between gap-4">
                    ...
                    <div
                      class="p-3 rounded-xl bg-slate-900/90 border border-slate-700 flex flex-col items-center justify-center shrink-0 min-w-[100px]"
                    >
                      <span class="text-2xl font-bold font-mono text-blue-400"
                        >{{ prof.sourceBackingRatio }}%</span
                      >
                      <span
                        class="text-[10px] font-bold uppercase text-slate-400 text-center font-mono"
                        >GROUNDED</span
                      >
                    </div>
                  </div>
                  ...
                  <div class="space-y-1.5 pt-3 border-t border-slate-700/50">
                    <span class="text-[11px] font-semibold text-slate-400"
                      >Grounded Technologies:</span
                    >
                    <div class="flex flex-wrap gap-1.5">
                      @for (tech of prof.technologies; track tech) {
                        <span
                          class="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-xs text-blue-300 font-mono"
                        >
                          {{ tech }}
                        </span>
                      }
              ...
              >
                ...
                >
                  ...
                  <span class="text-[10px] text-emerald-400 font-mono">SOURCE QUOTED</span>
                </div>
                  ...
                  @for (m of prof.metrics; track m.id) {
                    <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-700 space-y-2">
                      <div class="flex items-start justify-between gap-2">
                        ...
                        <span
                          class="px-2 py-0.5 text-xs font-bold font-mono text-emerald-400 bg-emerald-950/60 rounded border border-emerald-800/40"
                        >
                          {{ m.achievedValue }}
                        </span>
                      </div>
                      ...
                      <div class="flex items-center justify-between text-[11px] text-slate-400">
                        <span
                          >Baseline:
                          <strong class="text-slate-300 font-mono">{{
                            m.baselineValue
                          }}</strong></span
                        >
                        <span class="text-blue-300 font-bold font-mono">{{
                          m.percentageChange
                        }}</span>
                      </div>
                      ...
                      <div
                        class="p-2 rounded bg-slate-950 border border-slate-800 text-[11px] font-mono text-slate-300 italic"
                      >
                        "{{ m.quote }}"
                      </div>
                    </div>
              ...
              >
                ...
                >
                  ...
                  <span class="text-[10px] text-blue-300 font-mono">VERBATIM PROOF</span>
                </div>
                  ...
                  @for (act of prof.actions; track act.id) {
                    <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-700 space-y-2">
                      <div class="flex items-center justify-between gap-2">
                        ...
                        <span
                          class="px-2 py-0.5 text-[10px] font-bold font-mono rounded bg-blue-500/20 text-blue-300 border border-blue-500/30 shrink-0"
                        >
                      ...
                      <div
                        class="p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-[11px] font-mono text-slate-300 space-y-1"
                      >
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div
                  class="bg-slate-800/50 rounded-2xl border border-slate-700/50 overflow-hidden p-4 space-y-2 shadow-md"
                >
                  <h4 class="text-xs font-bold uppercase tracking-wider text-slate-300">
                    Key Constraints Overcome
                  </h4>
                  @if (prof.challenges.length > 0) {
                    ...
                    <p class="text-xs text-slate-500 leading-relaxed">
                      No explicit constraints found in source evidence.
                    </p>
                  }
                </div>
              ...
              >
                ...
                <div>
                  <h3 class="text-base font-bold text-white">
                    Extract Structured Evidence Profile
                  </h3>
                  <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto">
                    Use Gemini 2.5 Flash to analyze the preserved artifact, map direct quotes, and
