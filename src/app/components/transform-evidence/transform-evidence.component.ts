  ...
  template: `
    @if (vaultService.selectedArtifact(); as art) {
      <div class="space-y-6 pb-24">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700/50 pb-5">
          <div class="space-y-1">
            <div class="flex items-center gap-2">
              <a [routerLink]="['/evidence', art.id]" class="text-xs text-slate-400 hover:text-white flex items-center gap-1">
              ...
              <span class="text-xs font-semibold text-blue-400 font-mono">Resume Transformation Studio</span>
            </div>
            ...
            <p class="text-xs text-slate-400">
              Transform proof into executive resume bullets. Every generated claim is mapped directly to source line citations and requires explicit human approval.
            </p>
          </div>
          ...
          <a 
            ...
            class="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition-colors shrink-0 shadow-sm"
          >
        ...
        <div class="bg-slate-800/50 rounded-2xl p-4 sm:p-5 border border-slate-700/50 space-y-4 shadow-md">
          <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            
            <!-- Target Role & Framework Form -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
              <div class="space-y-1.5">
                <label class="text-xs font-semibold text-slate-300">Target Role Title for Resume Framing</label>
                <input 
                  type="text" 
                  [(ngModel)]="targetRole" 
                  placeholder="e.g. Staff Distributed Systems Engineer"
                  class="w-full px-3.5 py-2 rounded-lg bg-slate-900 border border-slate-700 focus:border-blue-500 text-xs text-white placeholder:text-slate-500 focus:outline-none"
                />
              </div>

              <div class="space-y-1.5">
                <label class="text-xs font-semibold text-slate-300">Resume Framing Formula</label>
                <select 
                  [(ngModel)]="selectedFramework"
                  class="w-full px-3.5 py-2 rounded-lg bg-slate-900 border border-slate-700 focus:border-blue-500 text-xs text-white focus:outline-none"
                >
                  <option value="XYZ (Accomplished [X] measured by [Y] by doing [Z])">Google XYZ Formula (Accomplished [X] as measured by [Y] by doing [Z])</option>
                  <option value="CAR (Context-Action-Result)">CAR Formula (Context, Action, Result)</option>
                  <option value="STAR (Situation-Task-Action-Result)">STAR Formula (Situation, Task, Action, Result)</option>
            ...
            >
              @if (geminiService.isGeneratingBullets()) {
                <svg class="w-4 h-4 animate-spin text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10"/></svg>
                <span>Grounding Claims with Gemini 3.7...</span>
              } @else {
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/></svg>
                <span>Generate Grounded Bullets</span>
              }
            </button>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <!-- Left Column: Generated Bullets with Review Controls (7 Cols) -->
            ...
            @if (getActiveTransformation(art); as trans) {
                ...
                @for (bullet of trans.bulletPoints; track bullet.id) {
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between gap-2">
                      <div class="flex items-center gap-2">
                        ...
                        @if (bullet.reviewStatus === 'approved') {
                          <span class="px-2.5 py-0.5 text-[10px] font-bold font-mono rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                            APPROVED BY AUTHOR
                          </span>
                        } @else if (bullet.reviewStatus === 'modified') {
                          <span class="px-2.5 py-0.5 text-[10px] font-bold font-mono rounded bg-blue-500/20 text-blue-300 border border-blue-500/30 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-400"></span>
                            EDITED & APPROVED
                          </span>
                        } @else if (bullet.reviewStatus === 'rejected') {
                          <span class="px-2.5 py-0.5 text-[10px] font-bold font-mono rounded bg-rose-500/20 text-rose-300 border border-rose-500/30 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-rose-400"></span>
                            REJECTED
                          </span>
                        } @else {
                          <span class="px-2.5 py-0.5 text-[10px] font-bold font-mono rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>
                            AWAITING HUMAN REVIEW
                          </span>
                        }

                        <span class="text-[10px] text-slate-500 font-mono">{{ bullet.framework }}</span>
                      </div>

                      <div class="flex items-center gap-1.5 text-xs font-mono">
                        <span class="text-slate-400 text-[10px]">Grounded:</span>
                        <span class="font-bold text-emerald-400">{{ bullet.overallConfidence }}%</span>
                      </div>
                    </div>
                    ...
                    @if (editingBulletId === bullet.id) {
                      ...
                      <div class="text-sm font-medium text-slate-100 leading-relaxed pl-3 border-l-2 border-blue-500">
                        {{ bullet.bulletText }}
                      </div>
                    }
                      ...
                      <div class="flex flex-wrap gap-1.5">
                        @for (claim of bullet.claims; track claim.claimId) {
                          <button 
                            ...
                            [ngClass]="selectedClaim()?.claimId === claim.claimId 
                          ...
                          >
                            <span class="w-1.5 h-1.5 rounded-full" [ngClass]="claim.sourceType === 'source_backed' ? 'bg-emerald-400' : 'bg-amber-400'"></span>
                            <span class="font-mono text-[10px] uppercase text-slate-400">[{{ claim.claimType }}]</span>
                            <span class="truncate max-w-[200px]">{{ claim.claimText }}</span>
                            <span class="text-[10px] font-mono text-slate-500 ml-1">{{ claim.confidenceScore }}%</span>
                          </button>
                        }
                      </div>
                    </div>
                    ...
                    <!-- User Notes (if any) -->
                    @if (bullet.userNotes) {
                      <div class="p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-xs text-slate-300 flex items-start gap-2">
                    ...
                    <div class="pt-2 border-t border-slate-700/50 flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div class="flex items-center gap-1.5">
                        ...
                        >
                          <svg class="w-3 h-3 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                          <span>Edit Text</span>
                        </button>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        >
                          <svg class="w-3 h-3 text-rose-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                          <span>Reject</span>
                        </button>
              ...
              <div class="bg-slate-800/50 rounded-2xl border border-slate-700/50 p-12 text-center space-y-4 shadow-md">
                <div class="w-12 h-12 rounded-2xl bg-blue-500/10 text-blue-400 flex items-center justify-center mx-auto">
                  <svg class="w-6 h-6 animate-pulse" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/></svg>
                </div>
                <div>
                  <h3 class="text-base font-bold text-white">Generate Verified Resume Transformation</h3>
                  <p class="text-xs text-slate-400 mt-1 max-w-md mx-auto">
                    Click "Generate Grounded Bullets" above to transform this {{ art.category }} proof into traceable executive resume bullets.
                  </p>
                </div>
                <button 
                  (click)="triggerNewTransformation(art.id)"
          ...
          <div class="lg:col-span-5 space-y-4">
            ...
            @if (selectedClaim(); as claim) {
              ...
              <div class="bg-slate-800/50 rounded-2xl p-5 border border-blue-500/50 bg-blue-950/15 space-y-4 shadow-md">
                
                <div class="flex items-start justify-between gap-2">
                  <div class="space-y-1">
                    <span class="px-2 py-0.5 text-[10px] font-bold font-mono uppercase rounded bg-blue-900/60 text-blue-300 border border-blue-700">
                  ...
                  <div class="text-right shrink-0">
                    <div class="text-xs font-bold font-mono text-emerald-400">{{ claim.confidenceScore }}%</div>
                ...
                <div class="flex items-center gap-2 text-xs">
                  ...
                  @if (claim.sourceType === 'source_backed') {
                    <span class="px-2 py-0.5 text-[11px] font-semibold font-mono text-emerald-300 bg-emerald-950/60 rounded border border-emerald-800/40 flex items-center gap-1">
                      <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                      Source-Backed (Primary Artifact)
                    </span>
                  } @else {
                    <span class="px-2 py-0.5 text-[11px] font-semibold font-mono text-amber-300 bg-amber-950/60 rounded border border-amber-800/40 flex items-center gap-1">
                ...
                <div class="space-y-1 pt-2 border-t border-slate-700/50">
                  <div class="flex items-center justify-between text-[11px]">
                    ...
                    <span class="text-slate-400 font-mono text-[10px]">{{ claim.lineReference || 'Artifact Context' }}</span>
                  </div>
                  
                  <div class="p-3 rounded-lg bg-slate-950 border border-emerald-500/40 text-xs font-mono text-emerald-300 leading-relaxed relative">
                    <div class="absolute -top-2 right-3 px-1.5 py-0.2 text-[9px] font-bold bg-emerald-900 text-emerald-300 rounded font-mono">
              ...
              <div class="bg-slate-800/50 rounded-2xl p-5 border border-slate-700/50 text-center space-y-2 shadow-md">
                <p class="text-xs text-slate-300 font-medium">Select any claim pill above</p>
                <p class="text-[11px] text-slate-500">
                  Credence will jump directly to the exact source quote proving that claim with confidence metrics.
                </p>
              </div>
            ...
            <div class="bg-slate-800/50 rounded-2xl border border-slate-700/50 overflow-hidden flex flex-col h-[460px] shadow-md">
              <div class="p-3 bg-slate-900/80 border-b border-slate-700/50 flex items-center justify-between text-[11px] font-mono">
                <span class="text-slate-300 font-medium truncate">{{ art.fileMetadata.fileName }}</span>
                <span class="text-emerald-400 font-bold shrink-0">SHA-256 VERIFIED</span>
              </div>

              <div class="p-4 overflow-y-auto flex-1 font-mono text-xs text-slate-300 leading-relaxed space-y-1 bg-slate-950/80">
                @for (line of getSourceLines(art.rawContentText); track $index) {
                  <div 
                    ...
                    [ngClass]="{
                      'bg-blue-950/90 border-l-2 border-blue-400 text-white font-semibold': isLineHighlighted(line),
                      'hover:bg-slate-900/50': !isLineHighlighted(line)
                    }"
                  >
                    <span class="text-[10px] text-slate-600 font-mono w-5 text-right shrink-0 select-none">
                      {{ $index + 1 }}
                    </span>
...
export class TransformEvidenceComponent implements OnInit {
  ...
  isLineHighlighted(line: string): boolean {
    ...
    return cleanQuote.includes(cleanLine) || cleanLine.includes(cleanQuote.substring(0, Math.min(20, cleanQuote.length)));
  }
  ...
  saveEditing(artifactId: string, transformationId: string, bulletId: string): void {
    if (this.tempEditText.trim()) {
      this.vaultService.editBulletText(artifactId, transformationId, bulletId, this.tempEditText.trim());
      this.editingBulletId = null;
    }
  }
