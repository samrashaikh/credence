...
@Component({
  ...
  template: `
    <header
      class="sticky top-0 z-40 w-full border-b border-slate-800 bg-slate-900/80 backdrop-blur-md"
    >
        ...
        <div class="flex items-center justify-between h-16 gap-4">
          ...
          <div class="flex items-center gap-6">
            <a routerLink="/" class="flex items-center gap-3 group">
              <div
                class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-white shadow-lg shadow-blue-900/30 ring-1 ring-white/10 group-hover:scale-105 transition-transform"
              >
                C
              </div>
              <div class="flex items-center">
                <span
                  class="text-xl font-semibold tracking-tight uppercase text-white border-l border-slate-700 pl-3 group-hover:text-blue-400 transition-colors"
                  >Credence</span
                >
            ...
            <nav class="hidden md:flex items-center gap-2">
              ...
              >
                ...
                @if (vaultService.vaultStats().pendingReviewCount > 0) {
                  <span
                    class="ml-1 px-1.5 py-0.2 text-[10px] font-bold rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30"
                  >
          ...
          <div class="flex items-center gap-3">
            ...
            <!-- Mobile Navigation -->
              <div class="relative md:hidden">
                <button
                  ...
                  class="flex items-center justify-center w-9 h-9 rounded-lg bg-slate-800/80 border border-slate-700 text-slate-300 hover:text-white hover:border-slate-600 transition-colors"
                ...
                @if (showMobileMenu) {
                  <div
                    class="absolute right-0 top-11 w-56 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl p-2 z-50"
                  >
                    <a
                      routerLink="/vault"
                      routerLinkActive="bg-slate-800 text-blue-400"
                      (click)="showMobileMenu = false"
                      class="block px-3 py-2 rounded-lg text-xs font-medium text-slate-300 hover:bg-slate-800 hover:text-white"
                    >
                      Evidence Vault
                    </a>
                    ...
                    <a
                      ...
                      routerLinkActive="bg-slate-800 text-blue-400"
                      (click)="showMobileMenu = false"
                      class="flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium text-slate-300 hover:bg-slate-800 hover:text-white"
                    >
            ...
            <div class="relative group">
              @if (firebaseService.currentUser(); as user) {
                ...
                >
                  <div
                    class="w-7 h-7 rounded-md bg-slate-700 border border-slate-600 text-white flex items-center justify-center text-xs font-bold"
                  >
                    {{ user.displayName.charAt(0) }}
                  </div>
                  <div class="hidden sm:flex flex-col pr-1">
                    <span
                      class="text-xs font-medium text-slate-200 truncate max-w-[130px] leading-tight"
                      >{{ user.displayName }}</span
                    >
                    <span
                      class="text-[10px] text-emerald-400 flex items-center gap-1 leading-tight"
                    >
                ...
                @if (showUserDropdown) {
                  <div
                    class="absolute right-0 mt-2 w-72 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl p-2 z-50 text-xs text-slate-300 animate-fade-in"
                  >
                    <div class="p-2 border-b border-slate-800 mb-1">
                      ...
                      <div
                        class="mt-2 flex items-center gap-1.5 text-[10px] text-blue-300 bg-blue-950/60 p-1.5 rounded border border-blue-800/40"
                      >
                        <svg
        ...
        <div
          ...
          [ngClass]="{
            'bg-slate-900/95 border-emerald-500/50 text-emerald-300 shadow-emerald-500/10':
              toast.type === 'success',
            'bg-slate-900/95 border-indigo-500/50 text-indigo-300 shadow-indigo-500/10':
              toast.type === 'info',
...
export class HeaderComponent {
  ...
  async signInWithGoogle(): Promise<void> {
    ...
    if (this.firebaseService.currentUser()) {
      this.vaultService.reloadForCurrentUser();
    }
  }

  async signOut(): Promise<void> {
    this.vaultService.clearSessionState();
